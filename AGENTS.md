...
- Read [SETUP.md](SETUP.md) before importing photos. Run `bun install` and `bun run sync:arena` to retrieve the Are.na images into ignored local assets. If given a channel export ZIP, use `bun run sync:arena --zip /path/to/export.zip`. After photos change, run `bun run score:colors`. Then run `bun run dev` or `bun run build`.
